...
<body>
    ...
    <div class="container py-5">
        ...
        @if(isset($device->table_name) && ($scheduleConfig ?? false))
            <div class="glass-card mt-4">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-calendar-range me-2"></i> Jadwal Irigasi
                        <small class="text-white-50 ms-2" style="font-size: 0.8rem;">(Dari Device)</small>
                    </h5>
                    <button type="button" class="btn btn-sm btn-success" onclick="openScheduleModal(0)">
                ...
                <div id="scheduleContainer" class="mt-3">
                    <div class="table-responsive">
                        <table class="table table-glass mb-0" id="scheduleTable">
                            <thead>
                                <tr>
                            ...
                            <tbody id="scheduleTableBody">
                                <tr>
                                    <td colspan="7" class="text-center text-white-50 py-3">
                    ...
                    <div class="modal-content"
                        ...
                        <div class="modal-header border-0">
                            <h5 class="modal-title text-white">
                                <i class="bi bi-gear me-2"></i>Set Jadwal - <span id="modalSlotTitle">Slot 1</span>
                            </h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <input type="hidden" id="scheduleSlotId" value="">

                            <div class="mb-3">
                                <label class="form-label text-white"><i class="bi bi-tag me-1"></i> Jenis</label>
                                <select id="scheduleType" class="form-select bg-dark text-white border-secondary">
                            ...
                            <div class="mb-3">
                                <label class="form-label text-white"><i class="bi bi-clock me-1"></i> Waktu Mulai</label>
                                <input type="time" id="scheduleTime"
                                    class="form-control bg-dark text-white border-secondary">
                            </div>

                            <div class="mb-3">
                                <label class="form-label text-white"><i class="bi bi-stopwatch me-1"></i> Durasi
                                    (menit)</label>
                                <input type="number" id="scheduleDuration"
                                    class="form-control bg-dark text-white border-secondary" min="1" max="60" value="5">
                            </div>

                            <div class="mb-3">
                                <label class="form-label text-white"><i class="bi bi-grid me-1"></i> Sektor</label>
                                <select id="scheduleSector" class="form-select bg-dark text-white border-secondary">
                                    @for($s = 1; $s <= ($scheduleConfig->max_sectors ?? 4); $s++)
                            ...
                            <div class="mb-3">
                                <label class="form-label text-white"><i class="bi bi-calendar-week me-1"></i> Hari</label>
                                <div class="d-flex flex-wrap gap-2">
                                    @foreach(['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'] as $day)
                                        <div class="form-check">
                                            <input class="form-check-input schedule-day" type="checkbox" value="{{ $day }}"
                                                id="day{{ $day }}">
                        ...
                        <div class="modal-footer border-0">
                            ...
                            <button type="button" class="btn btn-success" onclick="sendScheduleToDevice()">
    ...
    @if(!($isAdminView ?? false))
        ...
        <script>
            ...
            // Open schedule modal for adding/editing
            function openScheduleModal(slotId = 0, data = null) {
                const modal = new bootstrap.Modal(document.getElementById('scheduleModal'));
                
                // Reset form
                document.getElementById('scheduleSlotId').value = slotId;
                document.getElementById('modalSlotTitle').textContent = slotId > 0 ? `Slot ${slotId}` : 'Jadwal Baru';
                ...
                if (data) {
                    ...
                    document.getElementById('scheduleType').value = data.type || 'BAKU';
                    document.getElementById('scheduleTime').value = data.time || '';
                    document.getElementById('scheduleDuration').value = data.duration || 5;
                    document.getElementById('scheduleSector').value = data.sector || 1;
                    
                    // Parse and check days
                    if (data.days) {
                        const daysList = data.days.split(',').map(d => d.trim());
                    ...
                    document.getElementById('scheduleType').value = 'BAKU';
                    document.getElementById('scheduleTime').value = '';
                    document.getElementById('scheduleDuration').value = 5;
                    document.getElementById('scheduleSector').value = 1;
                }
                
                modal.show();
            }
            
            // Send schedule to device via AJAX
            async function sendScheduleToDevice() {
                const slotId = document.getElementById('scheduleSlotId').value;
                const scheduleType = document.getElementById('scheduleType').value;
                const scheduleTime = document.getElementById('scheduleTime').value;
                const scheduleDuration = document.getElementById('scheduleDuration').value;
                const scheduleSector = document.getElementById('scheduleSector').value;
                ...
                // Validation
                if (!scheduleTime) {
                    alert('Mohon isi waktu mulai!');
                    return;
                }
                if (!scheduleDuration || scheduleDuration < 1) {
                    alert('Mohon isi durasi yang valid!');
                    return;
                }
                if (selectedDays.length === 0) {
                    alert('Mohon pilih minimal satu hari!');
                    return;
                }
                
                // Determine slot ID - if 0, find next available slot
                let targetSlot = parseInt(slotId);
                if (targetSlot === 0) {
                    // Find next available slot from existing schedules
                    const existingSlots = document.querySelectorAll('#scheduleTableBody tr[data-slot]');
                    const usedSlots = Array.from(existingSlots).map(tr => parseInt(tr.dataset.slot));
                    for (let i = 1; i <= {{ $scheduleConfig->max_slots ?? 14 }}; i++) {
                    ...
                    if (targetSlot === 0) {
                        alert('Semua slot jadwal sudah terisi!');
                        return;
                    }
                }
                
                // Show loading
                const submitBtn = document.querySelector('#scheduleModal .btn-success');
                const originalText = submitBtn.innerHTML;
                submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span> Mengirim...';
                submitBtn.disabled = true;
                
                try {
                    const response = await fetch(`/device/${userDeviceId}/schedule/time`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
                        ...
                        body: JSON.stringify({
                            slot_id: targetSlot,
                            on_time: scheduleTime,
                            duration: parseInt(scheduleDuration),
                            days: selectedDays.join(''),
                            sector: parseInt(scheduleSector),
                    ...
                    if (data.success) {
                        ...
                        alert(data.message || 'Jadwal berhasil dikirim ke device!');
    ...
    <script>
        ...
        function updateSchedules(schedules) {
            ...
            schedules.forEach(sch => {
                const slotNum = parseInt(sch.key.replace('sch', ''));
                ...
                html += `
                    <tr data-slot="${slotNum}">
                        <td>
                            <span class="badge bg-primary">${slotNum}</span>
                        </td>
                        <td>
                            <span class="badge ${sch.name === 'PUPUK' ? 'bg-warning text-dark' : (sch.name === 'DRAIN' ? 'bg-info' : 'bg-success')}">
                                ${sch.name || 'Air Baku'}
                            </span>
                        </td>
                        <td>
                        ...
                        <td>
                            <i class="bi bi-hourglass-split me-1"></i>${sch.duration || 0} min
                        </td>
                        <td>
                            ${sch.sector > 0 ? `<span class="badge bg-warning text-dark">Sektor ${sch.sector}</span>` : '-'}
                        </td>
                        <td>
                            <small>${sch.days || '-'}</small>
                        </td>
                        <td>
                            <button type="button" class="btn btn-sm btn-outline-warning" 
                                    onclick='openScheduleModal(${slotNum}, ${scheduleData})' 
                                    title="Edit jadwal">
